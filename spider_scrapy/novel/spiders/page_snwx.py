...
class NovelSpider(CrawlSpider):
    ...
    intro_xpath = '//div[@class="intro"]/text()'
    ...
    def parse_item(self, response):
        ...
        if len(auth_label) > 0:
            item['auth'] = filter.get_label_value(auth_label[0].encode('utf-8'))
        else:
            item['auth'] = ""
        covers = response.xpath(self.cover_xpath).extract()
        if len(covers) > 0:
            item['cover'] = covers[0].encode('utf-8')
        else:
            item['cover'] = ""
        style_label = response.xpath(self.style_xpath).extract()
        if len(style_label) > 0:
            item['style'] = filter.get_label_value(style_label[0].encode('utf-8'))
        else:
            item['style'] = ""

        status_label = response.xpath(self.status_xpath).extract()
        if len(status_label) > 0:
            item['status'] = filter.get_label_value(status_label[0].encode('utf-8')).decode().replace("小说", "")
        else:
            item['status'] = ""

        intros = response.xpath(self.intro_xpath).extract()
        if len(intros) > 0:
            intro = intros[0].encode('utf-8').decode()
            item['intro'] = re.compile(r'<b>[^<]+</b>', re.S).sub("", intro)
        else:
            item['intro'] = ""
        return item
